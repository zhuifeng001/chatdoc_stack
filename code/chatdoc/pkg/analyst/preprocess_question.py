...
LastEditTime: 2025-03-05 19:55:20
'''
import os.path
from pkg.analyst.objects import Context, QuestionAnalysisResult
...
def preprocess_question(context: Context) -> Context:
    ...
    if question:
        ...
        all_files = get_files_by_uuid(context.params.document_uuids)
    ...
    if context.params.document_uuids:
        ...
        for company in company_entities:
            company_files = FileES().search_by_company(company.eid or company.uuid, size=1)
...
def file_match(question, file_metas: list[ESFileObject], question_years, regular_question_years, question_companys) -> list[str]:
    """
...
def file_location(companys: list[str], file_metas: list[ESFileObject], company_mapper: dict):
