...
LastEditTime: 2024-10-17 16:58:11
'''

import time

from pkg.doc.pdf2md import parse_document_new
...
from pkg.es.es_file import FileES, ESFileObject
...
def thread_process(context: Context) -> Context:
    ...
    try:
        ...
        # 文档解析
        # parser_engine = config['parse'].get('engine', 'doc_parser')
        # if parser_engine == 'doc_parser':
        #     context = doc_parse(context)
        #     # 目录树解析
        #     context = catalog(context)
        # elif parser_engine == 'pdf2md':
        #     # 兼容处理：如果 pdf2md 报错，则走doc_parse
        #     try:
        #         context = parse_document_new(context)
        #     except FileProcessException:
        #         # 如果报错，则强制doc_parse重新解析【临时方案，需要优化参数传递】
        #         context.params.force_doc_parse, tmp = True, context.params.force_doc_parse
        #         context = doc_parse(context)
        #         # 目录树解析
        #         context = catalog(context)
        #         context.params.force_doc_parse = tmp
        ...
        callback(context.params.callback_url, context.params.uuid, FileProcessStatus.file_process_error.value)
        raise e
    ...
    # 更新 es_file
    insert_file_bool = FileES().insert_file(context.es_file_entity)
    ...
    if not insert_file_bool or [thread_ret for thread_ret in thread_rets if thread_ret not in [None, True]]:
        ...
        logger.info(f"Doc Process Success, trace_id: {context.trace_id}")
        # 回调文件处理状态：切片成功
        ...
        # summary 处理
        # context = summary_document(context)
        # callback(context.params.callback_url, context.params.uuid, FileProcessStatus.file_summary_success.value)
...
def process(params: Params) -> Context:
    ...
    file_entities = FileES().get_by_file_uuids([context.params.uuid], with_doc_fragments_json=False)
...
def process_sync(params: Params) -> Context:
    ...
    file_entities = FileES().get_by_file_uuids([context.params.uuid])
...
def report_process_result(context: Context, status, file_entity: ESFileObject = None):
    file_meta = None
    if file_entity:
