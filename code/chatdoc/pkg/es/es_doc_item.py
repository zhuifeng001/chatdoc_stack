...
LastEditTime: 2024-10-16 10:57:40
'''
...
class DocItemES(object):

    @property
    def index_name(self):
        return config["es"]["index_doc_item"]
    ...
    def search_by_ori_id(self, uuid: str, ori_id: list[str], size: int = 10) -> list[DocItemModel]:
    ...
    def insert_doc_item(self, doc_item: DocItemModel) -> bool:
        """
        插入数据
        :param data:
        :return:
    ...
    def insert_doc_items(self, doc_items: list[DocItemModel]) -> bool:
        """
        插入数据
        :param data:
        :return:
    ...
    def delete_by_file_uuid(self, uuid, wait_delete=True):
        ...
        logger.info(f"DocItemES delete_by_file_uuid: {uuid}, cost: {1000*(time.time() - start_time):.1f}ms")
    ...
    def delete_by_file_uuids(self, uuids, wait_delete=True):
        ...
        logger.info(f"DocItemES delete_by_file_uuids: {uuids}, cost: {1000*(time.time() - start_time):.1f}ms")

    def get_by_uuid_ori_tuples(self, pairs: list[tuple[str, str]], fillup=True) -> list[DocItemModel]:
        ...
        search_body = {
            ...
            "size": int(len(pairs) * 2.4)
        }
        hits = global_es.search(index=self.index_name, search_body=search_body)

        doc_items = [DocItemModel(**hit["_source"]) for hit in hits]
        ...
        if fillup and len(doc_items) >= len(pairs) and uncached_pairs:
            # 再重新补全一遍, 确保找到
            fillup_doc_items = self.get_by_uuid_ori_tuples(uncached_pairs, fillup=False)
