...
LastEditTime: 2024-10-16 11:00:51
'''
...
class PESFileObject(EsBaseItem):
    user_id: str
    uuid: str
    ori_type: str
    filename: str
...
class PFileES(object):

    @ property
    def index_name(self):
        return config["es"]["index_p_file"]
    ...
    def insert_file(self, file: PESFileObject, wait_insert=True, wait_sec: int = 30, sleep_gap=0.5):
        """
        插入数据
        :param data:
        :return:
        """
        insert_succ = global_es.insert(self.index_name, docs=[
            {
        ...
        while time.time() < st + wait_sec:
            count = global_es.get_query_count(self.index_name, query)
            if count > 0:
                logger.info(f"wait_insert file succ: time cost is {1000*(time.time() - st):.1f}ms")
                return True
            time.sleep(sleep_gap)

        logger.error(f"Error: wait_insert file timeout: {query}")
        return False
    ...
    def insert_files(self, files: list[PESFileObject]):
        """
        插入数据
        :param data:
        :return:
    ...
    def search_by_company(self, company_eid: str = None, company_uuid: str = None, size: int = 10) -> list[PESFileObject]:
        """
        ...
        return [
            PESFileObject.from_hit(hit["_source"])
            for hit in hits
        ]
    ...
    def delete_by_user_and_file_uuids(self, user_id, uuids, wait_delete=True):
        ...
        logger.info(f"PFileES delete_by_user_and_file_uuid, user_id: {user_id}, uuids: {uuids}, cost: {1000*(time.time() - start_time):.1f}ms")

    def get_by_file_uuids(self, user_id, uuids, with_doc_fragments_json=True):
        hits = global_es.search(self.index_name, {
            "_source": PESFileObject.keys() if with_doc_fragments_json else PESFileObject.keys(exclude=["doc_fragments_json"]),
            "size": len(uuids),
            "query": {
                "bool": {
                    "must": [
    ...
    def search_file_brief_by_query(self, user_id, query) -> list[PESFileObject]:
        condition_should = [
            {
        ...
        hits = global_es.search(self.index_name, {
            "_source": PESFileObject.keys(exclude="doc_fragments_json"),
            "size": 100,
            "query": {
                "bool": {
    ...
    def search_file_fragment_json(self, user_id, uuid: str):
        hits = global_es.search(self.index_name, {
...
if __name__ == "__main__":
    objects = PFileES().search_by_company(company_eid="6d042872-a626-4174-aae9-f418e8e2af87")
    print(objects)
