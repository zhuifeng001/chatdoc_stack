...
LastEditTime: 2024-10-08 18:40:58
'''
...
def fillin_personal_doc_items_cache(doc_item_cache, user_id: str, uuid_ori_id_tuples: list[tuple[str, str]]) -> list[DocItemModel]:
    '''
    description: 填充doc_item的cache
    return {*}
    '''
    ...
    if to_request_dict:
        ...
        threads = []
        for i in range(0, len(to_request_list), max_batch_size):
            t = ThreadWithReturnValue(target=PDocItemES().get_by_uuid_ori_tuples, kwargs=dict(pairs=to_request_list[i:i + max_batch_size], user_id=user_id))
            t.start()
            threads.append(t)
...
def fillin_fragment_parent_cache(fragment_cache: dict[str, Union[PDocFragmentModel, DocFragmentModel]], fragments: list[DocFragmentModel], level: int = None):
    '''
...
def fillin_personal_fragment_parent_cache(fragment_cache: dict[str, Union[PDocFragmentModel, DocFragmentModel]], fragments: list[PDocFragmentModel], level: int = None):
    '''
    description: 填充节点的父节点的fragment_cache
    return {*}
    '''
    ...
    while _fragments:
        ...
        uncached_parent_uuids = [item.parent_frament_uuid for item in _fragments if item.parent_frament_uuid and item.parent_frament_uuid not in fragment_cache]
        uncached_parent_uuids = list(set(uncached_parent_uuids))
        _fragments = PDocFragmentES().get_by_uuids(uncached_parent_uuids) if uncached_parent_uuids else []
        level_cnt += 1


def fillin_fragment_children_cache(fragment_cache: dict[str, Union[PDocFragmentModel, DocFragmentModel]], fragments: list[DocFragmentModel]):
    '''
...
def fillin_personal_fragment_children_cache(fragment_cache: dict[str, Union[PDocFragmentModel, DocFragmentModel]], fragments: list[PDocFragmentModel]):
    '''
    description: 填充节点的子节点的fragment_cache，并返回相应的ori_id列表，用做uuid_ori_id缓存
    return {*}
    '''
    ...
    while _fragments:
        ...
        if uncached_children_uuids:
            uncached_children_uuids = list(set(uncached_children_uuids))
            children = PDocFragmentES().get_by_uuids(uncached_children_uuids)
            for item in children:
                fragment_cache[item.uuid] = item
...
def get_fragment_ori_ids(fragment: Union[DocFragmentModel, PDocFragmentModel], fragment_cache: dict[str, Union[DocFragmentModel, PDocFragmentModel]]):
    '''
...
def get_fragment_ori_text(fragment: Union[DocFragmentModel, PDocFragmentModel],
                          fragment_cache: dict[str, Union[DocFragmentModel, PDocFragmentModel]],
    ...
    return {*}
    '''
    if isinstance(fragment, PDocFragmentModel):
        doc_items_cache = p_doc_items_cache
    ...
    child_ori_texts = [
        get_fragment_ori_text(fragment_cache[child_uuid], fragment_cache, doc_items_cache, p_doc_items_cache)
...
def get_fragment_all_texts(fragment: Union[DocFragmentModel, PDocFragmentModel], fragment_cache: dict[str, DocFragmentModel], doc_items_cache: dict[str, DocItemModel], p_doc_items_cache: dict[str, PDocItemModel]):
    '''
    ...
    for child_uuid in fragment.children_fragment_uuids:
        if fragment_cache[child_uuid].parent_frament_uuid == fragment.uuid:
            children_texts.extend(get_fragment_all_texts(fragment_cache[child_uuid], fragment_cache, doc_items_cache, p_doc_items_cache))
...
def get_table_ori_text(doc_table: Union[DocTableModel, PDocTableModel] , doc_ori_cache: dict[str, DocItemModel], p_doc_ori_cache: dict[str, DocItemModel]):
    '''
    description: 通过缓存获取 table的text， 补充填满【fragment_cache中包含该节点的所有子孙节点的Fragment】，doc_items_cache中也包含了【ori_id的缓存】
    return {*}
    '''

    if isinstance(doc_table, PDocTableModel):
        doc_ori_cache = p_doc_ori_cache
