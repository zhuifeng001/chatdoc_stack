...
LastEditTime: 2024-10-22 11:17:25
'''
...
class RetrieveContext(BaseModel):
    ...
    # 召回来源
    origin: typing.Union[PDocTableModel, PDocFragmentModel] = None
    ...
    # 相关的子召回列表
    related: list[typing.Union[PDocTableModel, PDocFragmentModel]] = []
    ...
    @property
    def file_uuid(self):
        if isinstance(self.origin, PDocTableModel):
            return self.origin.uuid

        elif isinstance(self.origin, PDocFragmentModel):
...
class Context(BaseModel):
    ...
    # 定位到的文件列表
    files: list[PESFileObject] = []
    locationfiles: list[PESFileObject] = []

    # 召回内容
    fixed_table_retrieve_small: list[PDocTableModel] = []
    normal_table_retrieve_small: list[PDocTableModel] = []
    fragment_retrieve_small: list[PDocFragmentModel] = []

    # 原文Cache [文件uuid|文件ori_id, PDocItemModel]
    doc_items_cache: dict[str, PDocItemModel] = {}
    # 片段Cache [片段uuid, PDocFragmentModel]
    fragment_cache: dict[str, PDocFragmentModel] = {}
