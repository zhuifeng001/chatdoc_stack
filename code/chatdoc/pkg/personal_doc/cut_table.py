...
LastEditTime: 2024-10-25 17:00:54
'''
...
def cut_table_fragment(context: Context) -> Context:
    ...
    thread = ThreadWithReturnValue(target=embedding_and_upload, args=(context.doc_table_row_items, context.params.uuid, context.params.user_id))
    thread.start()
...
def embedding_and_upload(doc_table_row_items: list[DocTableRowItem], file_uuid: str, user_id: str):
    from pkg.es.es_p_doc_table import PDocTableES, PDocTableModel

    start_time = time.time()

    # 删除老切片
    PDocTableES().delete_by_user_and_file_uuids(user_id, [file_uuid], wait_delete=True)

    if not doc_table_row_items:
        logger.warning(f"PFile cut has 0 table, file_uuid: {file_uuid}, user_id: {user_id}")
        return True

    success = PDocTableES().insert_doc_tables([
        PDocTableModel(
            user_id=user_id,
            uuid=file_uuid,
            title=item.title,
    ...
    if not success:
        logger.error(f"PTable Update Es Not Success, file_uuid: {file_uuid}, user_id: {user_id}")
    else:
        logger.info(f"PTable Update Es All Successed, file_uuid: {file_uuid}, user_id: {user_id},  docs: {len(doc_table_row_items)}, Elapsed: {1000*(time.time() - start_time):.1f}ms")
