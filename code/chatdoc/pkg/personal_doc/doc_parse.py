...
LastEditTime: 2024-10-16 14:24:01
'''
...
def doc_parse(context: Context) -> Context:
    ...
    if not context.params.force_doc_parse and local_or_remote_exist(context.doc_parse_path):
        ...
        thread = ThreadWithReturnValue(target=upload_doc_parser, args=(doc_result_json, context.params.user_id, context.doc_parse_path,))
        thread.start()
...
def upload_doc_parser(doc_result: str, user_id: str, docpath: str, url=None):
    compress_data = compress(doc_result)
    doc_parser_name = docpath.split("/")[-1]
    doc_parser_compress_name = f"User_{user_id}/" + doc_parser_name.replace(".json", ".gz")
