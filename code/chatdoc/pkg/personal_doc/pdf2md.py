...
from pkg.personal_doc.md2tree import TreeBuild, build_tree_by_page, detail_process, tree_generate
from pkg.storage import Storage
from pkg.utils import compress, retry_exponential_backoff, xjson
from pkg.utils.decorators import register_span_func

from pkg.personal_doc.objects import Context, DocTree, FileProcessException
from pkg.config import BASE_DIR, config
from pkg.personal_doc.doc_parse import local_or_remote_exist as local_or_remote_exist_doc_parse, upload_doc_parser
from pkg.personal_doc.catalog import local_or_remote_exist as local_or_remote_exist_catalog, upload_catalog
...
def handle_doc_parse(context: Context, res_json_all):
    ...
    upload_doc_parse_thread = ThreadWithReturnValue(target=upload_doc_parser, args=(doc_result_json, context.params.user_id, context.doc_parse_path))
    upload_doc_parse_thread.start()
    context.threads.append(upload_doc_parse_thread)

    upload_brief_thread = ThreadWithReturnValue(target=upload_brief, args=(metrics, context.params.user_id, context.params.uuid))
    upload_brief_thread.start()
    context.threads.append(upload_brief_thread)

    # 异步上传文件图片【异步去做，不需要加入到context.threads当中】
    upload_imgs_thread = ThreadWithReturnValue(target=upload_imgs, args=(metrics, context.params.user_id, context.params.uuid))
    upload_imgs_thread.start()


def handle_catalog(context: Context, res_json_all):
    md_detail = res_json_all["result"]["detail"]
    page_metrics = res_json_all["metrics"]

    # 条件：文件中大部分为扁文件，认为是PPT
    ...
    if is_ppt:
        ...
        context.doc_tree = DocTree(tree=[tree], generate=[])
    else:
    ...
    upload_catalog_thread = ThreadWithReturnValue(target=upload_catalog, args=(context.params.user_id, context.doc_tree.model_dump_json(include=["generate"]), context.catalog_path_frontend))
    upload_catalog_thread.start()
    context.threads.append(upload_catalog_thread)

    upload_catalog_backend_thread = ThreadWithReturnValue(target=upload_catalog, args=(context.params.user_id, context.doc_tree.model_dump_json(), context.catalog_path))
    upload_catalog_backend_thread.start()
    context.threads.append(upload_catalog_backend_thread)


def upload_imgs(metrics: list, user_id: str, uuid: str):
    ...
    if resp_body and "pics" in resp_body:
        ...
        Storage.upload(f"User_{user_id}/brief-{uuid}.gz", compress(content))
        logger.info(f"Backend Backup Image Success: {uuid}, User: {user_id}, length: {len(resp_body['pics'])}")

    else:
        logger.error(f"Backend Backup Image Failed: {uuid}, User: {user_id}")


def upload_brief(metrics: list, user_id: str, uuid: str):
    pics = [
        dict(
            dpi=page.get("dpi", None),
            angle=page.get("angle", 0),
